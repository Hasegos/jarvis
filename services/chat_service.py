...
from services.agent.agent_service import chat_with_tools, stream_chat_with_tools
...
async def _prepare_turn(
    ...
    블로킹/스트리밍 파이프라인이 동일하게 거치는 단계
    (세션 판단 → 히스토리 → user 임베딩 → 프로필/위키/RAG 컨텍스트)를
    한곳에 모아 두 파이프라인의 중복을 제거한다.
    ...
    Returns:
        ...
        - session       : 현재 세션 객체
        - history       : user 턴이 추가된 대화 히스토리
        - user_embedding: 현재 입력 임베딩 (저장 시 재사용)
        - context       : 프로필+위키+RAG 합본 블록. 없으면 None.
        - embed_sec     : user 임베딩 소요 시간(초)
    Raises:
        RuntimeError: 임베딩 오류
    """
    # ──────────────────────────────────────
...
async def process_message_stream(
    ...
    # 5-1. 공통 턴 준비
    # ──────────────────────────────────────
    session, history, user_embedding, context, _embed_sec = await _prepare_turn(
        db, session_id, user_text
    )
